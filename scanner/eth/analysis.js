...
class TEMP extends ANALYSIS {
    constructor() {
        super('ETH', 'BLOCK_ETH', new Web3(new Web3.providers.HttpProvider(CONFIG.provider.eth.host)));
    }
    ...
    async handleLogs(transactions, logs, timestamp) {
        ...
        for (let log of logs) {
            ...
            if (analysis.length <= 0) {
                console.log(`${this.chain} NO transfers`)
                continue;
            }
            let data = analysis[0]
    ...
    async handlePunk(transactions,logs,timestamp){
        ...
        for (let log of logs) {
            ...
            let cryptopunks='0xb47e3cd837ddf8e4c57f05d70ab865de6e193bbb';
