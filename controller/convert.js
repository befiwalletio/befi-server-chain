...
class Convert {
    ...
    async parseSymbol(symbol, contractAddress) {
        if (symbol === "ETH") return {'lib': 'ETH'}
        if (symbol === "MATIC") return {'lib': 'MATIC'}
        if (symbol === "BNB") return {'lib': 'BNB'}
        if (symbol === "TRUE") return {'lib': 'TRUE'}
        if (symbol === "TRX") return {'lib': 'TRX'}
        let coin = await MYSQL.BEEPAYRDS().get('tokens', {
            contract: contractAddress
        });
        if (coin) {
            return {'lib': coin.contract, 'contractAddress': contractAddress}
        } else {
            this.error(20002)
        }
    }

    async parseChain(contract) {
        if (contract){
            contract=contract.toUpperCase();
        }
        if (contract === "ETH") return {'lib': 'ETH'}
        if (contract === "MATIC") return {'lib': 'MATIC'}
        if (contract === "BNB" || contract === "BSC") return {'lib': 'BNB'}
        if (contract === "TRUE") return {'lib': 'TRUE'}
        if (contract === "TRX") return {'lib': 'TRX'}
        this.error(20002)
    }
