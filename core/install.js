...
function installMongo(){
    const chains = ['BNB','TRUE','ETH','MATIC','KLAY'];
    ...
    function createMongo(name,config){
        ...
        for (let chain of chains) {
            ...
            mongoose.model(`BLOCK_${chain}`,new mongoose.Schema(schema));
        }
...
function installMysql(){
    ...
    function createMysql(name, config) {
        ...
        // global['MYSQL'][dbname+"RDS"] = rds(config)
        global['MYSQL'][dbname+"RDS"] =  function (transactions=false){
            if (global['MYSQL']['CACHE']['RDS']){
                return global['MYSQL']['CACHE']['RDS'];
            }
            global['MYSQL']['CACHE']['RDS']=rds(config);
            return global['MYSQL']['CACHE']['RDS'];
        };
